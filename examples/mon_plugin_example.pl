...
qq{-w, --warning=INTEGER
   a result greater than or equal to the given warning threshold,
   and less than the critical threshold, will generate a WARNING
   error message},
#   required => 1,
   default => 1
);
...
qq{-c, --critical=INTEGER
   a result greater than or equal to the critical threshold will generate a
